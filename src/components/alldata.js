...
import { Table, Input, Spin, message, Button } from 'antd';
...
function Alldata(props) {
  ...
  const fetchData = () => {
    setLoading(true);
    setTimeout(() => {
      fetch('http://192.168.1.13:3002/amount/search')
        .then(response => {
          if (!response.ok) {
        ...
        .then(data => {
          ...
          if (searchQuery.trim() === '') {
            setCut(false);
            // const filteredData = data.filter(item => item.iopy == 'No');
            setRawData(data); 
          } else {
    ...
    <div className='approved-main'>
      <div className='approved-header'>
        ...
        <div className='search-approved'>
          ...
          <span onClick={clearsearch} style={{ position: "relative", cursor: 'pointer', right: '35px', color: '#5365cf', fontSize: '18px', top: '2px' }}>
